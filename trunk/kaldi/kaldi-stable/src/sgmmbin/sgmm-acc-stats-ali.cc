  ...
  try {
    ...
    for (; !feature_reader.Done(); feature_reader.Next()) {
      ...
      if (!alignments_reader.HasKey(utt)) {
        ...
        // The size of the alignment i.e. the length of the utterance
        ...
        for (size_t i = 0; i < alignment.size(); i++) {
          // The alignment consists of a vector of transtion ids
          int32 tid = alignment[i],  // transition identifier.
              pdf_id = trans_model.TransitionIdToPdf(tid);
          // if transitions need to be accumulated
          if (acc_flags & kaldi::kSgmmTransitions) 
            // We increment the count of the transition id by 1.0
          ...
          // The following function must be called with each frame
          // prior to accumulation or likelihood evaluation.
          // This function pre computes certain quantities 
          // xti, zti, nt, xt as mentioned in the SGMM paper
          // xt     : x'(t), FMLLR-adapted, Dim=[D], eq 33
          // xti    : x_{i}(t) = x'(t)-o_i(s), Dim=[I][D], eq 34 
          // the speaker adapted version. i.e removing o_i(s)=N_i.v(s)
          // zti    : z_{i}(t), Dim=[I][S], eq 35
          // nti    : n_{i}(t), Dim=[I], eq 36
