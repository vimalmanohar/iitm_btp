// phoneTxCATbin/phoneTxCAT-align-compiled.cc
...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        "Align features given [PhoneTxCAT-based] models.\n"
        "Usage: phoneTxCAT-align-compiled [options] model-in graphs-rspecifier "
        "feature-rspecifier alignments-wspecifier\n"
        "e.g.: phoneTxCAT-align-compiled 1.mdl ark:graphs.fsts scp:train.scp ark:1.ali\n";
    ...
    for (; !fst_reader.Done(); fst_reader.Next()) {
      ...
      if (!feature_reader.HasKey(utt)) num_no_feat++;
        ...
        DecodableAmPhoneTxCATScaled phoneTxCAT_decodable(phoneTxCAT_opts, am_phoneTxCAT, trans_model,
            features, *gselect, log_prune, acoustic_scale);

        decoder.Decode(&phoneTxCAT_decodable);
