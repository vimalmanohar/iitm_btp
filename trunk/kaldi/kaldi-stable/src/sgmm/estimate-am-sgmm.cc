...
BaseFloat MleAmSgmmAccs::Accumulate(const AmSgmm &model,
  ...
  // Compute posteriors using eq. 37-38
...
double MleAmSgmmUpdater::UpdatePhoneVectorsChecked(const MleAmSgmmAccs &accs,
  ...
  for (int32 j = 0; j < accs.num_states_; j++) {
    for (int32 m = 0; m < model->NumSubstates(j); m++) {
      double gamma_jm = accs.gamma_[j].Row(m).Sum(); // Eq. 56
      SpMatrix<double> X_jm(accs.phn_space_dim_);  // = \sum_i \gamma_{jmi} H_i as in Eq. 59
      ...
      Vector<double> v_jm_orig(model->v_[j].Row(m)),
          v_jm(v_jm_orig);  // Create copies of the model parameters
      ...
      for (backtrack_iter = 0; backtrack_iter < max_backtrack; backtrack_iter++) {
        ...
        // This is the exact objective function.
        // sum_it gamma_jmi*w_jmi*(o(t)-mu_jmi)^T*Sinv_i*(o(t)-mu_jmi)
        ...
        if (backtrack_iter == 0) {  // computing updated value.
          w_jm.ApplyExp();  // it is now w_jm. log w_jm is converted to w_jm
          ...
          // Eq. 60. This does a complex operation to calculate H_jm^-1*g_jm
          auxf_impr =
...
double MleAmSgmmUpdater::UpdateVars(const MleAmSgmmAccs &accs,
  ...
  if (gamma_vec.Sum() == 0) {  // If no count, use identity.
    ...
    // scale the accumulated covfloor by f/ (sum_i gamma_i) Pg. 26
