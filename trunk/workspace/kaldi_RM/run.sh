...
local/rm_data_prep.sh $DATA_DIR || exit 1;
...
featdir=~/scratch/RM_kaldi/features

for x in test_mar87 test_oct87 test_feb89 test_oct89 test_feb91 test_sep92 train; do
  steps/make_mfcc.sh --nj $NP --cmd $run_cmd data/$x exp/make_mfcc/$x $featdir  || exit 1;
...
steps/train_mono.sh --nj $NP --cmd "$train_cmd" --num-iters 1 data/train.1k data/lang exp/mono  || exit 1;
exit 1
...
steps/decode.sh --config conf/decode.config --nj $NP --cmd "$decode_cmd" \
  exp/mono/graph data/test exp/mono/decode
...
steps/align_si.sh --nj $NP --cmd "$train_cmd" \
  data/train data/lang exp/mono exp/mono_ali || exit 1;
...
steps/decode.sh --config conf/decode.config --iter 4 --nj $NP --cmd "$decode_cmd" \
   exp/tri2b/graph data/test exp/tri2b_mmi/decode_it4
#steps/decode.sh --config conf/decode.config --iter 3 --nj 20 --cmd "$decode_cmd" \
#   exp/tri2b/graph data/test exp/tri2b_mmi/decode_it3
steps/decode.sh --config conf/decode.config --iter 3 --nj $NP --cmd "$decode_cmd" \
   exp/tri2b/graph data/test exp/tri2b_mmi/decode_it3
...
steps/decode_fmllr.sh --config conf/decode.config --nj $NP --cmd "$decode_cmd" \
  exp/tri3b/graph data/test exp/tri3b/decode || exit 1;
...
for iter in 3 4 5 6 7 8; do
 ...
 steps/decode_fmmi.sh --nj $NP --config conf/decode.config --cmd "$decode_cmd" --iter $iter \
   --transform-dir exp/tri3b/decode  exp/tri3b/graph data/test exp/tri3b_fmmi_b/decode_it$iter &
done
