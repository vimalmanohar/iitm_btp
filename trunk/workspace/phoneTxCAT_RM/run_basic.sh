...
if [ $prepareTrainFlag == 1 ]; then
    ...
    bash -x utils/prepare_lang.sh --position-dependent-phones false data/local/dict '!SIL' data/local/lang data/lang || exit 1;
...
if [ $extractFeaturesFlag == 1 ]; then
    featdir=$TEMP_DIR/features_dict
...
if [ $monoTrainFlag == 1 ]; then
    ...
    steps/train_mono.sh --stage $mono_stage --nj $NP_train --cmd "$train_cmd" data/train.1k data/lang $monoDir || exit 1;
