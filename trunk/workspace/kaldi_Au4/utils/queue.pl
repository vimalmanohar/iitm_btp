...
$template = "$dir/q" . "_tempDir_" . "XXXXXX";
$qdir = tempdir($template);
...
if (defined $jobname) { # It's an array job.
  $numjobs=$jobend-$jobstart+1;
  $numnodes=int($numjobs/16)+1;
  #$queue_array_opt = "-l nodes=${numnodes}:ppn=16 -t ${jobstart}-${jobend}";#:${max_jobs}"; 
  $queue_array_opt = "-t ${jobstart}-${jobend}";#:${max_jobs}"; 
...
print Q ") >$logfile\n";
print Q " ( $cmd ) 2>>$logfile.err >>$logfile\n";
print Q "ret=\$?\n";
...
#$queue_errfile="$queue_logfile.err";
#print("qsub -S /bin/bash -v PATH -j oe -o $queue_logfile $qsub_opts $queue_array_opt $queue_scriptfile >>$queue_logfile 2>&1\n");
print("qsub -S /bin/bash -v PATH -k oe -o $queue_logfile $qsub_opts $queue_array_opt $queue_scriptfile >>$queue_logfile 2>&1\n");
#$ret = system ("qsub -S /bin/bash -v PATH -cwd -j y -o $queue_logfile $qsub_opts $queue_array_opt $queue_scriptfile >>$queue_logfile 2>&1");
$ret = system ("qsub -S /bin/bash -v PATH -k oe -o $queue_logfile $qsub_opts $queue_array_opt $queue_scriptfile >>$queue_logfile 2>&1");
#print("qsub -S /bin/bash -v PATH -o $queue_logfile -e $queue_errfile $qsub_opts $queue_array_opt $queue_scriptfile >>$queue_logfile 2>>$queue_errfile");
#$ret = system ("qsub -S /bin/bash -v PATH -o $queue_logfile -e $queue_errfile $qsub_opts $queue_array_opt $queue_scriptfile >>$queue_logfile 2>>$queue_errfile");
if ($ret != 0) {
...
$jobno=`cat $queue_logfile | sed 's/\\([0-9]*\\).*/\\1/g'`;
chomp($jobno);
print ("Job number: $jobno\n");

system("mv \$HOME/*sh*[oe]$jobno $qdir >> $queue_logfile");
