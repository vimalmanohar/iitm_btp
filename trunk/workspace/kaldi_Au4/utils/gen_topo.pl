...
if(@ARGV != 4 and @ARGV !=5) {
  print STDERR "Usage: utils/gen_topo.pl <num-nonsilence-states> <num-silence-states> <colon-separated-nonsilence-phones> <colon-separated-silence-phones> [<use_htk_sil_topo>]\n";
...
if(@ARGV == 4) {
  ...
  ($num_nonsil_states, $num_sil_states, $nonsil_phones, $sil_phones, $use_htk_sil_topo) = @ARGV;
}
...
$nonsil_phones =~ s/:/ /g;
$sil_phones =~ s/:/ /g;
$nonsil_phones =~ m/^\d[ \d]*$/ || die "$0: bad arguments @ARGV, $nonsil_phones\n";
$sil_phones =~ m/^\d[ \d]*$/ || die "$0: bad arguments @ARGV, $sil_phones\n";
...
if (!$use_htk_sil_topo) {
  ...
  $transp2 = 1.0 / ($num_sil_states);
  for ($nextstate = 0; $nextstate < $num_sil_states; $nextstate++) { # Transitions to all 
    # emitting state.
    print "<Transition> $nextstate $transp2 ";
  }
}
print "</State>\n";
...
if (!$use_htk_sil_topo) {
  ...
  print "<State> $state <PdfClass> $state <Transition> 0 0.25 <Transition> $state 0.5 <Transition> $num_sil_states 0.25 </State>\n";
}
