...
case $feat_type in
  ...
  if [[ -f $train_config ]]; then
      ...
      feats="ark,s,cs:apply-cmvn --norm-vars=false --utt2spk=ark:$sdata/JOB/utt2spk scp:$sdata/JOB/cmvn.scp scp:$sdata/JOB/feats.scp ark:- | add-deltas --config=$train_config ark:- ark:- |"
...
while [ $x -lt $num_iters ]; do
   ...
   if [ $x -eq 0 ]; then
     flags=vwct # on the first iteration, don't update projections M or N
...
if [ $spk_dim -gt 0 ]; then
  ...
  while [ $x -lt $[$num_iters+$num_iters_alimdl] ]; do
    ...
    if [ $x -eq $num_iters ]; then # 1st pass of building alimdl.
      flags=Mwc # don't update v the first time.  Note-- we never update transitions.
