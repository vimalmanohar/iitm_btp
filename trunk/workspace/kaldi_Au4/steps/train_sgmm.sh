...
spk_dim=0  # You can use this to set the speaker subspace dim. [default: feat-dim]
power=0.25 # Exponent for number of gaussians according to occurrence counts
beam=8
retry_beam=40
cluster_thresh=-1  # for build-tree control final bottom-up clustering of leaves
feat_config=conf/feat.config
cov_diag_ratio=2 # Use 10000 for diag
full_gselect=15
gselect=50
use_no_substates=true
...
if [ $stage -le -3 ]; then
  ...
  $cmd JOB=1:$nj $dir/log/gselect.JOB.log \
    sgmm-gselect --diag-gmm-nbest=$gselect --full-gmm-nbest=$full_gselect $dir/0.mdl "$feats" \
...
while [ $x -lt $num_iters ]; do
   ...
   if [ $stage -le $x ]; then
     ...
     $cmd $dir/log/sum_accs.$x.log \
       sgmm-sum-accs $dir/$x.accs $dir/$x.*.acc || exit 1
      rm $dir/$x.*.acc 
   fi
   ...
   if [ $use_no_substates ]; then
     split_substates_opts=
   else
     split_substates_opts="--split-substates=$numsubstates" 
   fi
   if [ $stage -le $x ]; then
     run.pl $dir/log/update.$x.log \
       sgmm-est --update-flags=$flags --cov-diag-ratio=$cov_diag_ratio $split_substates_opts $increase_dim_opts \
         --power=$power --write-occs=$dir/$[$x+1].occs $dir/$x.mdl $dir/$x.accs \
       $dir/$[$x+1].mdl || exit 1;
  ...
  while [ $x -lt $[$num_iters+$num_iters_alimdl] ]; do
    ...
    if [ $stage -le $x ]; then
      ...
      $cmd $dir/log/update_ali.$x.log \
        sgmm-est --update-flags=$flags --cov-diag-ratio=$cov_diag_ratio --remove-speaker-space=true --power=$power $cur_alimdl \
