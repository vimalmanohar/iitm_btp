...
cdhmmName=tri1.vimal.htk1.6000.24000.$train_cond
ubmName=ubm.vimal.htk1.delta3.$train_cond
#ubmName=ubm.vimal.htkUBM.tri1.allUBM.delta3.$train_cond
...
sgmmExptName=sgmm.vimal.diag1.cdhmm3.ubm3.tri1.sgmm3.6000.24000.${initSubStates}.${finalSubStates}.$train_cond.realign5_10_15.$num_iters
...
if [ $sgmmTrainFlag == 1 ]; then
    ...
    if [ $noCovarUpdateFlag == 1 ]; then
        ...
        steps/train_sgmm.sh --cmd "$train_cmd" \
            --feat_config $sgmm_feat_config --num_iters $num_iters \
            --realign_iters '5 10 15' \
            --stage $stage --cov-diag-ratio $cov_diag_ratio $initSubStates $finalSubStates \
            data/train_$train_cond data/lang \
            ${cdhmmDir}_ali $ubmDir/final.ubm $sgmmDir || exit 1;
