...
monoName=mono.vimal.dict1
tri1Name=tri1.vimal.dict1.$initGaussians.$finalGaussians
triDeltasName=tri2a.vimal.dict1.$initGaussians.$finalGaussians
...
if [ $prepareTrainFlag == 1 ]; then
    ...
    bash -x utils/prepare_lang.sh --position-dependent-phones false data/local/dict '!SIL' data/local/lang data/lang --use-htk-sil-topo true || exit 1;
...
if [ $monoTrainFlag == 1 ]; then
    ...
    steps/train_mono.sh --stage 39 --nj $NP_train --cmd "$train_cmd" data/train.1k data/lang $monoDir || exit 1;
    ...
    utils/mkgraph.sh --mono data/lang $monoDir $monoDir/graph
...
if [ $triTestFlag == 1 ]; then
    ...
    # decode tri1
    utils/mkgraph.sh data/lang $tri1Dir $tri1Dir/graph || exit 1;
    #steps/decode.sh --config $decode_config --nj 20 --cmd "$decode_cmd" \
